...
module.exports = function(options) {
  ...
  options.jsDocFilters = (options.jsDocFilters) ? options.jsDocFilters : [];
  ...
  for (var i = 0; i < apiPaths.length; i = i + 1) {
    ...
    //Always run builtin filter first
    var swaggerJsDocComments = filterJsDocComments(files.jsdoc);

    options.jsDocFilters.forEach( function(filter) {
      swaggerJsDocComments = filter.filterJsDocComments(swaggerJsDocComments, options)
    });
